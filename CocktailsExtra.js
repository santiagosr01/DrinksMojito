...
const mojitos = {
  ...
  const nombreMojito = prompt("Introduce el nombre del mojito:");
  
  for (let i = 0; i < mojitos.drinks.length; i++) {
    const mojito = mojitos.drinks[i];
  
    if (mojito.strDrink.toLowerCase() === nombreMojito.toLowerCase()) {
    
      console.log(`Nombre: ${mojito.strDrink}`);
      console.log("Ingredientes:");
      
      for (let j = 1; j <= 15; j++) {
        const ingrediente = mojito[`strIngredient${j}`
      ];
        const medida = mojito[`strMeasure${j}`
      ];
        if (!ingrediente) {
          break;
      }
        console.log(`${ingrediente}: ${medida || 'al gusto'}`);
    }
      break;
  }
}
  console.log(`No se encontró el mojito "${nombreMojito}" en la base de datos.`);
